...
variance <- function(x, rep) (1/(rep-1)) * sum((x - mean(x))^2)
...
create_figure2 <- function(){
  ...
  for(i in 1:nrow(main_scenarios_2)){
    ...
    output[i,"variance_gamma_0"] <- variance(results[,"effect_crude"], rep = 1000)
    output[i,"variance_gamma_1"] <- variance(results[,"effect_splines"], rep = 1000)
  }
  ...
  for(j in 1:length(beta1)){
    input_plot[j,"variance_gamma_0_n500"] <- mean(output[output[,"nobs"]==500 & output[,"beta1"] == exp(beta1[j]),"variance_gamma_0"])
    input_plot[j,"variance_gamma_1_n500"] <- mean(output[output[,"nobs"]==500 & output[,"beta1"] == exp(beta1[j]),"variance_gamma_1"])
    input_plot[j,"variance_gamma_0_n2500"] <- mean(output[output[,"nobs"]==2500 & output[,"beta1"] == exp(beta1[j]),"variance_gamma_0"])
    input_plot[j,"variance_gamma_1_n2500"] <- mean(output[output[,"nobs"]==2500 & output[,"beta1"] == exp(beta1[j]),"variance_gamma_1"])
  ...
  plot(x = input_plot[,"beta1"],
       ...
       type = "l",
       lty = 4,
       xlim=c(min(exp(beta1)),max(exp(beta1))),
       ylim = c(0,0.007),
...
create_figure3 <- function(){
  ...
  grid_contour_n500 <- expand.grid(alpha1 = exp(alpha1), beta1= exp(beta1))
  ...
  grid_contour_n2500 <- expand.grid(alpha1 = exp(alpha1), beta1= exp(beta1))
  ...
  par(mar=c(0,0,0,0))
  par(mfrow=c(1,2))
  contour(x = exp(alpha1), y = exp(beta1), z = input_contour_n500, levels = c(0.5,0.9,1.1,1.5),axes = FALSE)
  abline(h = 3.5)
  axis(1)
  axis(2)
  axis(3, labels = F)
  ...
  contour(x = exp(alpha1), y = exp(beta1), z = input_contour_n2500, levels = c(0.5,0.9,1.1,1.5), axes = FALSE)
  abline(v = c(1,3.5))
  abline(h = 3.5)
  axis(1, labels = F)
  axis(3)
...
create_figure4 <- function(){
  ...
  for(i in 1:nrow(main_scenarios_2)){
    ...
    output[i,"mse_gamma_0"] <- mse(results[,"effect_crude"])
    output[i,"mse_gamma_1"] <- mse(results[,"effect_quintiles"])
  }
